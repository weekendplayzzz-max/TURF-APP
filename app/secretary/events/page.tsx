...
export default function SecretaryEvents() {
  ...
  useEffect(() => {
    if (role === 'secretary') { fetchEvents(); fetchLinkedGuests(); }
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        transaction.set(doc(collection(db, 'eventParticipants')), {
          ...
          playerEmail: user.email || '', playerType: 'regular', playerRole: 'secretary',
              ...
              {linkedGuests.length > 0 && (
                <>
                  ...
                  {linkedGuests.map(guest => (
                    <div key={guest.guestId} onClick={() => toggleGuest(guest.guestId)}
            ...
            {filteredEvents.map(event => {
              ...
              return (
                <div key={event.id} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                  ...
                  {/* Status color bar */}
                  <div className={`h-1 w-full ${event.status === 'open' ? 'bg-red-500' : 'bg-gray-300'}`} />
