  ...
  useEffect(() => {
    const fetchStatistics = async () => {
      ...
      try {
        ...
        // Query events collection for this secretary
        const eventsRef = collection(db, 'events');
        const secretaryQuery = query(
          eventsRef,
          where('secretaryId', '==', user.uid)
        );
        const eventsSnapshot = await getDocs(secretaryQuery);

        const eventsOrganized = eventsSnapshot.size;
        ...
        eventsSnapshot.forEach((doc) => {
          ...
          // Count as completed if event status is not open
          if (data.status !== 'open') {
            eventsCompleted++;
          }

          // Add total collections for closed events
              ...
              <div className="flex items-start gap-3 sm:gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 mb-1 truncate">
                    {user.displayName || user.email?.split('@')[0] || 'Secretary'}
                  </h2>
            ...
            {loadingStats ? (
              ...
              <div className="grid grid-cols-3 gap-3 sm:gap-6">
                <div className="text-center p-3 sm:p-6 bg-gray-50 rounded-lg sm:rounded-xl border border-gray-200">
                  ...
                  <p className="text-xs sm:text-sm text-gray-600 font-medium">Events Organized</p>
                </div>
                <div className="text-center p-3 sm:p-6 bg-gray-50 rounded-lg sm:rounded-xl border border-gray-200">
                  <p className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-1 sm:mb-2">
                    {statistics.eventsCompleted}
                  </p>
                  <p className="text-xs sm:text-sm text-gray-600 font-medium">Events Completed</p>
                </div>
                <div className="text-center p-3 sm:p-6 bg-gray-50 rounded-lg sm:rounded-xl border border-gray-200">
                  <p className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-1 sm:mb-2">
                    ₹{statistics.totalCollections.toLocaleString()}
                  </p>
                  <p className="text-xs sm:text-sm text-gray-600 font-medium">Total Collections</p>
                </div>
              </div>
            )}
          </div>
