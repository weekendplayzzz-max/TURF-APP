...
export default function MyPayments() {
  ...
  useEffect(() => {
    // Allow player, secretary, and treasurer roles
    if (!loading && role !== 'player' && role !== 'secretary' && role !== 'treasurer') {
      router.push('/login');
    }
  }, [role, loading, router]);

  useEffect(() => {
    if ((role === 'player' || role === 'secretary' || role === 'treasurer') && user) {
      fetchMyPayments();
    }
  }, [role, user]);
  ...
  if (loading || !user || (role !== 'player' && role !== 'secretary' && role !== 'treasurer')) {
    return (
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="bg-white rounded-lg p-3 border border-red-200">
                  ...
                  <p className="text-xs sm:text-sm text-gray-600">
                    {role === 'treasurer' 
                      ? 'Complete your payments to maintain financial transparency. You can self-mark payments from the treasurer dashboard.'
                      : role === 'secretary' 
