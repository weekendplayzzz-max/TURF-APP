...
const ALLOWED_ROLES = ['player', 'secretary', 'treasurer'];
...
export default function MyPayments() {
  ...
  useEffect(() => {
    if (!loading && (!role || !ALLOWED_ROLES.includes(role))) router.push('/login');
  }, [role, loading, router]);

  useEffect(() => {
    if (role && ALLOWED_ROLES.includes(role) && user) fetchMyPayments();
  }, [role, user]);
  ...
  const fetchMyPayments = async () => {
    ...
    try {
      setLoadingData(true);
      const snap = await getDocs(collection(db, 'eventPayments'));
      const list: Payment[] = [];
      snap.forEach(d => {
        const data = d.data();
  ...
  const totalAmountDue   = payments
    ...
    .reduce((s, p) => s + Math.max(0, p.currentAmountDue - p.totalPaid), 0);
  ...
  if (loading || !user || !role || !ALLOWED_ROLES.includes(role)) {
    return (
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                ...
                <div key={label} className="bg-white/[0.07] rounded-xl px-2 py-2.5 text-center border border-white/10">
                  ...
                  <p className="text-sm font-black text-white mt-1 leading-none">{value}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="flex items-start gap-2.5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-gray-500 mt-0.5 leading-relaxed">
                  {role === 'treasurer'
                    ? 'You can self-mark payments from the treasurer dashboard.'
                    : role === 'secretary'
                    ? 'Contact your treasurer or mark from the treasurer dashboard.'
                    : 'Contact your treasurer to settle. Status updates automatically once confirmed.'}
                </p>
              </div>
            </div>
          </div>
        ...
        {loadingData ? (
          ...
          <div className="bg-white rounded-3xl border border-gray-100 p-10 text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">
              {filter === 'all' ? 'Join events to see your payment history' : filter === 'paid' ? 'Paid payments will appear here' : 'No pending payments'}
            </p>
          </div>
        ) : (
                ...
                <div key={eventId} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="flex-1 min-w-0">
                        <h3 className="text-sm font-black text-gray-900 leading-snug">{first.eventTitle}</h3>
                    ...
                    <div className="space-y-2">
                      {eventPayments.map(payment => {
                        ...
                        const isMyGuest = payment.parentId === user.uid && payment.playerId !== user.uid;
                        ...
                        return (
                          <div key={payment.id} className={`rounded-2xl border p-3 ${
                              ...
                              <div className="flex items-center gap-2 min-w-0">
                                ...
                                <div className="min-w-0">
                                  <div className="flex items-center gap-1.5 flex-wrap">
                                    <p className="text-xs font-bold text-gray-900 leading-tight">{payment.playerName}</p>
                                    {isGuest && (
                                      <span className="text-[9px] font-black px-1.5 py-0.5 bg-gray-200 text-gray-500 rounded-full leading-none">
                                        {isMyGuest ? 'Your Guest' : 'Guest'}
                                      </span>
                                    )}
                                  </div>
                                </div>
                            ...
                            {/* Due / Paid / Balance row */}
                            <div className="flex items-center divide-x divide-gray-200 bg-white rounded-xl border border-gray-100 overflow-hidden">
                              {[
                            ...
                            {isPaid && payment.paidAt && (
                              <div className="mt-2.5 flex items-center gap-1.5">
                                ...
                                <p className="text-[10px] text-gray-400">
                                  Confirmed{' '}
                                  {payment.paidAt.toDate().toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
                                  {payment.markedPaidByName && (
                                    <> by <span className="font-bold text-gray-600">{payment.markedPaidByName}</span></>
                                  )}
                                </p>
                              </div>
                            )}
                    ...
                    {/* Multi-participant event total */}
                    {eventPayments.length > 1 && (
                      <div className="mt-3 flex items-center gap-0 divide-x divide-gray-200 bg-gray-50 rounded-2xl border border-gray-100 overflow-hidden">
                        {[
                          { label: 'Total Due',  value: `₹${totalEventDue}`  },
                          { label: 'Total Paid', value: `₹${totalEventPaid}` },
                          { label: 'Balance',    value: `₹${totalEventBal}`, red: totalEventBal > 0 },
                        ].map(({ label, value, red }) => (
                          <div key={label} className="flex-1 py-2 text-center">
                            <p className="text-[9px] text-gray-400 font-semibold uppercase tracking-wide">{label}</p>
                            <p className={`text-xs font-black mt-0.5 ${red ? 'text-red-600' : 'text-gray-700'}`}>{value}</p>
                          </div>
                        ))}
                      </div>
                    )}
