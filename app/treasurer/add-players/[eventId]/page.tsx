...
const handleAddPlayers = async () => {
  ...
  if (!confirm(`Add ${selectedPlayers.size} player(s) to this event? This will recalculate all payments.`)) {
    return;
  }
  ...
  try {
    ...
    for (const playerId of selectedPlayers) {
      const player = appointedPlayers.find((p) => p.id === playerId);
      if (!player) continue;
    ...
    // ✅ FIX: Update current participants state immediately
    ...
    // ✅ FIX: Update event state immediately with new count
    setEvent(prevEvent => {
    ...
    // ✅ FIX: Remove added players from appointed players list
    setAppointedPlayers(prev => 
    ...
    setMessage(`✅ Successfully added ${selectedPlayers.size} player(s) and recalculated payments`);
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 mb-8 border border-gray-200">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-600">{event?.participantCount}</p>
                </div>
                <div>
                ...
                <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
                  <div className="flex gap-4">
                    <button
                      ...
                      className="flex-1 px-6 py-4 bg-blue-600 text-white font-bold text-lg rounded-lg hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {processing
                        ? '⏳ Adding Players...'
