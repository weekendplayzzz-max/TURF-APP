...
export default function PlayerEventParticipants() {
  ...
  const regularList = participants.filter(p => p.playerType !== 'guest');
  const guestList   = participants.filter(p => p.playerType === 'guest');
      ...
      <div className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-40">
        <div className="max-w-lg mx-auto px-4 py-3 flex items-center gap-3">
          ...
          {event?.status === 'open' && !loadingData && (
            ...
            >
              <svg className={`w-4 h-4 text-gray-600 ${refreshing ? 'animate-spin' : ''}`}
            ...
            {event?.status === 'open' && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3 flex items-center gap-3 animate-slideDown">
                ...
                <p className="text-xs font-semibold text-gray-600 flex-1">
                  Live list - Click refresh icon in header to update
                </p>
              </div>
            )}
                      ...
                      <div key={participant.id}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] text-gray-400 mt-0.5">
                            Joined {participant.joinedAt?.toDate().toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
